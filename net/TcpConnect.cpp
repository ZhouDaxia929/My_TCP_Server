...
void TcpConnect::setWriteCompleteCallback(const OnWriteCompleteCallback callback){
...
void TcpConnect::writeEvent(){
    if(event->isWriting()){
        ...
        base::Log::OUT(base::Log::Warn) << "Connection fd = " << std::to_string(event->getFd()) 
...
void TcpConnect::write(const void *data, uint32_t length){
    ...
    //如果该写数据缓冲区内有数据，直接写入socket缓冲区会导致数据交叠
...
void TcpConnect::writeInLoop(const void *data, uint32_t len){
    loop->runInLoop([this, data, len] {write(data, len);});
}
